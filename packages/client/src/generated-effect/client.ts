...
type Endpoint3_14Request = Parameters<RawClient["server.session"]["session.research"]>[0]
type Endpoint3_14Input = {
  readonly sessionID: Endpoint3_14Request["params"]["sessionID"]
  readonly question: Endpoint3_14Request["payload"]["question"]
  readonly profile?: Endpoint3_14Request["payload"]["profile"]
  readonly path?: Endpoint3_14Request["payload"]["path"]
}
const Endpoint3_14 = (raw: RawClient["server.session"]) => (input: Endpoint3_14Input) =>
  raw["session.research"]({
    params: { sessionID: input["sessionID"] },
    payload: { question: input["question"], profile: input["profile"], path: input["path"] },
  }).pipe(
    Effect.mapError(mapClientError),
    Effect.map((value) => value.data),
  )
